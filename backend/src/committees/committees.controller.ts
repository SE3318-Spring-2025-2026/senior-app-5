...
export class CommitteesController {
  ...
  @ApiOperation({
    operationId: 'deleteCommittee',
    summary: 'Delete a committee and all its assignment links (COORDINATOR only)',
    description: 'Deletes the committee. Advisor and Group entities are NOT deleted — groups return to the unassigned pool.',
  })
  @ApiNoContentResponse({ description: 'Committee deleted successfully' })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid JWT' })
  @ApiForbiddenResponse({ description: 'Valid token but insufficient permissions' })
  @ApiNotFoundResponse({ description: 'Committee not found' })
  @ApiInternalServerErrorResponse({ description: 'Unexpected internal failure' })
  ...
  async deleteCommittee(
    ...
    const coordinatorId = req.user.userId ?? req.user.sub ?? req.user._id ?? 'unknown';
    const correlationId = (req.headers['x-correlation-id'] as string) ?? undefined;
    await this.committeesService.deleteCommittee(committeeId, coordinatorId, correlationId);
  }
  ...
  @ApiCreatedResponse({ description: 'Jury member added successfully', type: JuryMemberResponseDto })
  @ApiConflictResponse({ description: 'User is already a jury member on this committee' })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid JWT' })
  @ApiForbiddenResponse({ description: 'Valid token but insufficient permissions' })
  @ApiNotFoundResponse({ description: 'Committee not found' })
  @ApiInternalServerErrorResponse({ description: 'Unexpected internal failure' })
  ...
  async addJuryMember(
    ...
    const coordinatorId = req.user.userId ?? req.user.sub ?? req.user._id ?? 'unknown';
    const correlationId = (req.headers['x-correlation-id'] as string) ?? undefined;
    return this.committeesService.addJuryMember(committeeId, dto, coordinatorId, correlationId);
  }
