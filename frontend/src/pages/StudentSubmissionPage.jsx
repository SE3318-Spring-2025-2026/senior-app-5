      ...
      <section className={styles.card}>
        ...
        <form onSubmit={handleSubmit} className={styles.formGrid}>
          ...
          <div className={styles.formRow}>
            ...
            <div
              className={`${styles.dropZone} ${isDragging ? styles.dropZoneActive : ''} ${isSubmissionDisabled ? styles.dropZoneDisabled : ''}`}
              onDragOver={(e) => { e.preventDefault(); if (!isSubmissionDisabled) setIsDragging(true); }}
              onDragLeave={() => setIsDragging(false)}
              onDrop={(e) => {
                e.preventDefault();
                setIsDragging(false);
                if (!isSubmissionDisabled) setFile(e.dataTransfer.files?.[0] || null);
              }}
              onClick={() => !isSubmissionDisabled && fileInputRef.current?.click()}
            >
              {file ? (
                ...
                <div className={styles.dropZonePrompt}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/></svg>
                  <span>Drag &amp; drop or <strong>click to browse</strong></span>
                  <small>PDF, DOC, DOCX, PNG, JPG, JPEG &mdash; max 5 MB</small>
                </div>
              )}
            </div>
            <input
